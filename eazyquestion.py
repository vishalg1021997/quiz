...
class eazyques:
    def __init__(self):
        self.Beginner_list = {}       #To Store Quiz Details
        self.topic = ""

    def beginnerequiz(self):         #To Start Creating Quiz
        if self.topic == "":
            print("Enter The Topic Name")
            a=input()
            self.topic = a
        que = input("Enter The Question \n")
        self.Beginner_list[que] = []     #Making Question As Key So Each Question Will Be Diffrent
        print(yellow+"Enter Four Option")
        for i in range(4):
            option  = input()
            self.Beginner_list[que].append(option)   #Appending The Four Option For The Question
        print(yellow+"Enter the Correct Answer --> A/B/C/D")
        corans = input()
        self.Beginner_list[que].append(corans)       #Taking And Storing The Correct Quiz
        print()

    def start_beginner(self):            #To Start The Quiz
        self.count = 0                   #To Count The Score
        answers = []                     #To Store The Answer Given BY USer
        for i in self.Beginner_list.items():  #Iterating Through The Set Of question And displaying Only 1
    ...
    def display_beginner(self):                 #To Display The Correct Answer
        for i in self.Beginner_list.items():    #Iterating Through The Set Of question And displaying Only 1
